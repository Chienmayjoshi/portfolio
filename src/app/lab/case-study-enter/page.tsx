...
// TEMPORARY tuning harness for the case study enter animation (Figma node
// 7438:44091). Deliberately NOT on /fastrouter: the real route stays untouched
// until the motion is signed off, so nothing shipped can regress while this is
// being dialled in. Delete this whole `src/app/lab/` directory when the
// interaction moves onto the real card -> case study flow.
//
// The snap target below is the actual Hero/MetaBar/CockpitPlaceholder, imported
// straight from components/fastrouter - not a copy. That's the point: whatever
// lands correctly here lands identically on the real page.

// Figma frames 1-3. Note this is NOT the hero's line break - the hero reads
// ".../ flying blind on every model / decision." The word sequence is the same
// either way, which is what makes index pairing valid.
const OVERLAY_LINES = [
  "Enterprise AI teams were",
  "flying blind on every",
  "model decision.",
];
  ...
  return (
    <>
      ...
      {/* Same column shape as src/app/fastrouter/page.tsx so the target lands
          at exactly the coordinates it will on the real page. */}
      ...
      <div className="fixed right-16px top-80px z-[70] w-[280px] font-mono text-[11px] leading-[16px]">
        <div className="flex gap-8px">
          ...
          >
            {running ? "Playing…" : "Replay"} <span className="text-text-muted">(space)</span>
          </button>
          <button
            onClick={() => setOpen((o) => !o)}
